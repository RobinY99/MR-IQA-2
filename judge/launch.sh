...
FLASHINFER_NVCC=""
JUDGER_MODEL_ID="${JUDGER_MODEL_ID:-${JUDGE_MODEL_ID:-source-e5-judge-step725}}"
JUDGER_MODEL_PATH="${JUDGER_MODEL_PATH:-${JUDGE_MODEL_PATH:-}}"
JUDGER_MANIFEST_PATH="${JUDGER_MANIFEST_PATH:-${JUDGE_MANIFEST_PATH:-}}"
JUDGER_MODEL_TREE_SHA256="${JUDGER_MODEL_TREE_SHA256:-${JUDGE_MODEL_TREE_SHA256:-}}"
JUDGER_MODEL_EXPORT_TREE_SHA256="${JUDGER_MODEL_EXPORT_TREE_SHA256:-${JUDGE_MODEL_EXPORT_TREE_SHA256:-}}"
JUDGER_PROMPT_HASH="${JUDGER_PROMPT_HASH:-${JUDGE_PROMPT_HASH:-}}"
JUDGER_PROMPT_SCHEMA="${JUDGER_PROMPT_SCHEMA:-${JUDGE_PROMPT_SCHEMA:-e5_training_reasoning_v5}}"
JUDGER_BACKEND="${JUDGER_BACKEND:-${JUDGE_BACKEND:-e5_qwen35_4b_vllm_judge}}"
[[ -n "${JUDGER_MODEL_PATH}" ]] || { echo "JUDGE_MODEL_PATH is required" >&2; exit 2; }
[[ -n "${JUDGER_MANIFEST_PATH}" ]] || { echo "JUDGE_MANIFEST_PATH is required" >&2; exit 2; }
[[ -n "${JUDGER_MODEL_TREE_SHA256}" ]] || { echo "JUDGE_MODEL_TREE_SHA256 is required" >&2; exit 2; }
[[ -n "${JUDGER_MODEL_EXPORT_TREE_SHA256}" ]] || { echo "JUDGE_MODEL_EXPORT_TREE_SHA256 is required" >&2; exit 2; }
[[ -n "${JUDGER_PROMPT_HASH}" ]] || { echo "JUDGE_PROMPT_HASH is required" >&2; exit 2; }
